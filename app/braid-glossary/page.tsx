          ...
          <div className="max-w-4xl mx-auto">
            <div className="columns-2 gap-6 space-y-6">
              {filteredBraids.map((braid) => {
                ...
                return (
                  <div key={braid.id} className="relative">
                    ...
                    {!isTextSubmission ? (
                      <>
                        {hasMultipleImages ? (
                          <>
                            <img
                              src={(braid as any).image_urls[currentImageIndex] || "/placeholder.svg"}
                              alt={braid.braid_name}
                              className="w-full h-auto rounded-none cursor-pointer hover:opacity-70 transition-opacity"
                              onClick={() =>
                                handleImageClick((braid as any).image_urls[currentImageIndex], braid.braid_name)
                              }
                            />
                            {totalImages > 1 && (
                                ...
                                <button
                                  onClick={(e) => {
                                    ...
                                    setCurrentImageIndices((prev) => ({
                                      ...prev,
                                      [braid.id.toString()]: (currentImageIndex - 1 + totalImages) % totalImages,
                                    }))
                                  }}
                                  className="bg-black bg-opacity-50 text-white text-xs px-2 py-1 rounded-full hover:bg-opacity-70 transition-colors"
                                >
                                  {"<"}
                                </button>
                                <button
                                  onClick={(e) => {
                                    ...
                                    setCurrentImageIndices((prev) => ({
                                      ...prev,
                                      [braid.id.toString()]: (currentImageIndex + 1) % totalImages,
                                    }))
                                  }}
                                  className="bg-black bg-opacity-50 text-white text-xs px-2 py-1 rounded-full hover:bg-opacity-70 transition-colors"
                                >
                      ...
                      <a
                        ...
                        className="block w-full h-auto rounded-none hover:opacity-70 transition-opacity"
                      >
                        <div className="bg-white p-4">
                          <h3 className="text-lg font-semibold">{braid.link_title}</h3>
                          <p className="text-gray-600">{braid.link_description}</p>
                        </div>
                      </a>
                    ) : (
                      <div className="bg-white p-4">
                        <h3 className="text-lg font-semibold">{braid.memory_title}</h3>
                        {braid.audio_url && (
                          <div className="mt-2">
                            <button
                              onClick={() => toggleAudio(braid.id, braid.audio_url!)}
                              className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition-colors"
                            >
                              {playingAudio[braid.id.toString()] ? "Pause Pronunciation" : "Play Pronunciation"}
                            </button>
                            {audioDurations[braid.id.toString()] && (
                              <span className="ml-2 text-gray-600">({audioDurations[braid.id.toString()]})</span>
                            )}
                            <audio
                              ref={(el) => (audioRefs.current[braid.id.toString()] = el)}
                              src={braid.audio_url}
                              style={{ display: "none" }}
                    ...
                    {/* Braid Name and Details */}
                    <div className="absolute bottom-0 left-0 w-full bg-black bg-opacity-50 text-white p-2">
                      <h2 className="text-sm font-semibold">{braid.braid_name}</h2>
                      <button onClick={() => setShowDetailModal(braid)} className="text-xs hover:underline">
                        View Details
                      </button>
                    </div>
                  </div>
